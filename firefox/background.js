/**
 * Firefox Background Script - Book More Extension
...
browser.runtime.onInstalled.addListener(async (details) => {
  ...
  // Open options on install/update
...
/**
 * Set up periodic sync using alarms (Firefox extension alternative to Chrome alarms)
 */
async function setupPeriodicSync() {
  // Firefox doesn't have the alarms API in the same way, so we'll use a different approach
  // Check sync time on each message
  setInterval(async () => {
    const storage = await browser.storage.local.get(STORAGE_KEYS.LAST_SYNC);
    const lastSync = storage[STORAGE_KEYS.LAST_SYNC];
    
    if (!lastSync || (Date.now() - new Date(lastSync).getTime() > SYNC_INTERVAL_MINUTES * 60 * 1000)) {
      console.log('[Background] Periodic sync triggered');
      const urls = await syncUrlsFromWorker();
      if (urls) {
        // Refresh active URL in background
        await browser.storage.local.remove([STORAGE_KEYS.ACTIVE_URL, STORAGE_KEYS.LAST_HEALTH_CHECK]);
      }
    }
  ...
  (async () => {
    try {
      switch (request.action) {
        ...
        case 'setCustomUrl':
          if (request.url) {
            await browser.storage.local.set({ [STORAGE_KEYS.CUSTOM_URL]: request.url });
            // Clear active to force re-check
            await browser.storage.local.remove([STORAGE_KEYS.ACTIVE_URL, STORAGE_KEYS.LAST_HEALTH_CHECK]);
            sendResponse({ success: true });
          } else {
            await browser.storage.local.remove(STORAGE_KEYS.CUSTOM_URL);
