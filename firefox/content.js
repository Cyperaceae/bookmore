(function () {
  ...
  /**
   * Get the effective URL from background service worker
   * Handles health checking and priority: customUrl > best remote mirror
   * @returns {Promise<string|null>} - The effective URL or null if none available
   */
  async function getEffectiveUrl() {
    try {
      // Ask background worker for the best URL (handles health checking internally)
      const response = await browser.runtime.sendMessage({ action: 'getEffectiveUrl' });

      if (response.success && response.url) {
        console.log('[Content] Using effective URL:', response.url);
        return response.url;
      }

      console.error('[Content] No effective URL available from background');
      return null;
  ...
  /**
   * Build search URLs using the effective Anna's Archive domain
   * @param {string} baseUrl - The Anna's Archive base URL
   * @param {string} title - Book title
   * @param {string|null} isbn - Book ISBN (optional)
